...
import { getPlatformIdsString, normalizePlatforms } from "../lib/handlePlatforms";
...
export class SearchGameController {
  static async handle({ queryParams, userId }: ProtectedIGDBHttpRequest): Promise<HttpResponse> {
    ...
    const fieldQuery = "fields id, name, platforms.name, cover.image_id, release_dates.y, release_dates.platform, release_dates.human;"
    const limitQuery = "limit 30;"

    console.log(`${searchQuery} ${whereQuery} ${fieldQuery} ${limitQuery}`)
    ...
    const formattedGames = games.map((g: GameIGDBResponse) => {
      const cover = g.cover?.image_id
        ...
        : null
      const normalizedPlatforms = normalizePlatforms(g.platforms, g.release_dates)
